 ...
 * 3- 阻塞性质：一旦创建这样的“有界缓存区“，就不能再增加其容量。试图向已满队列中放入元素会导致操作受阻塞；试图从空队列中提取元素将导致类似阻塞。
 * 4- 线程排序：此类支持对等待的生产者线程和消费者线程进行排序的可选公平策略。默认情况下，不保证是这种排序。（这里指线程排序）
 ...
 *  	创建一个具有给定的（固定）容量和指定公平策略的 ArrayBlockingQueue。
 *  
 *  ArrayBlockingQueue(int capacity, boolean fair, Collection<? extends E> c) 
 *  	创建一个具有给定的（固定）容量和指定公平策略的 ArrayBlockingQueue，它最初包含给定 collection 的元素，并以 collection 迭代器的遍历顺序添加元素。
 *   
 *  重要方法
 *  	add(E e) 
