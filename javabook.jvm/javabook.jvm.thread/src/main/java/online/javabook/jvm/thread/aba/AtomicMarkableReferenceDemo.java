...
public final class AtomicMarkableReferenceDemo {
	 ...
	 */
	static AtomicMarkableReference<String> envelope = new AtomicMarkableReference<String>("A", false);
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Thread t1 = new Thread(new Runnable() {
			...
			public void run() {
				...
				// T1尝试给‘远方的来信’回信
				boolean result = envelope.compareAndSet(mailContent, "A",
		...
		Thread t2 = new Thread(new Runnable() {
			...
			public void run() {
				...
				boolean firstOpt = envelope.compareAndSet(mailContent, "B",
						markHolder[0], !markHolder[0]);
				if (firstOpt)
				...
				boolean secondOpt = envelope.compareAndSet("B", "A",
						markHolder[0], !markHolder[0]);
				if (secondOpt)
					System.out.println("T2悄悄把信封里的邮件内容[ "
							+ envelope.getReference() + " ]放回。"
