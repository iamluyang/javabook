...
import online.javabook.jvm.thread.counter.api.SimpleLock;

import java.util.concurrent.atomic.AtomicBoolean;

/**
 * 与{@link TaompTASLock}效果等价的基于testAndGet()的锁算法。
 */
public class TaompTTASLock implements SimpleLock {

    private AtomicBoolean state = new AtomicBoolean(false);

    /**
     * 相比{@link TaompTASLock#lock()}的实现，TTASLock的lock方法实现加上了判断是否当前锁被占用，避免每次都直接执行getAndSet。
